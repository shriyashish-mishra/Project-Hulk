...
/**
 * Maps a free-text muscle name from an AI report to zero or more diagram
 * regions — the coarser 9-region model used for the distribution list and
 * "most/least trained" insights (the map itself highlights the finer
 * 14-group model in `muscleGroups.ts`). Ported verbatim from the web
 * app's src/lib/progress/muscle-map.ts.
 */
...
export function computeRegionCounts(muscleNamesByDay: string[][]): Map<MuscleRegion, number> {
