 ...
 * Ported verbatim from the web app's src/lib/progress/muscle-groups.ts —
 * keep the two in sync if the mapping ever changes on either side.
 */
...
export function computeMuscleGroupCounts(musclesTrainedByDay: string[][]): Map<MuscleGroupId, number> {
...
export function computeMuscleGroupIntensity(musclesTrainedByDay: string[][]): Partial<Record<MuscleGroupId, number>> {
