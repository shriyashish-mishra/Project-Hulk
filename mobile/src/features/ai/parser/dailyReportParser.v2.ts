...
function optionalExercises(value: unknown): AIReportExercise[] | undefined {
  if (!Array.isArray(value)) return undefined;
  const exercises = value
    .map((item): AIReportExercise | null => {
      if (typeof item !== 'object' || item === null) return null;
      const record = item as Record<string, unknown>;
      const name = optionalString(record.name);
      if (!name) return null;
      const detail = optionalString(record.detail);
      const caloriesBurned = optionalNumber(record.caloriesBurned);
      return { name, ...(detail ? { detail } : {}), ...(caloriesBurned !== undefined ? { caloriesBurned } : {}) };
    })
    .filter((item): item is AIReportExercise => item !== null);
  return exercises.length > 0 ? exercises : undefined;
}

/**
 * Turns the raw text a user pastes back from Claude into `AIReportContent`
 * — matched to what `dailyReport.v2`'s prompt asked for. Never throws: a
 * manual copy/paste workflow will always see the occasional malformed or
 * off-shape reply, and losing what the user pasted would be worse than a
 * best-effort, partially-defaulted parse. The v2-only fields (calories,
 * macros, muscles trained, exercises) are all optional — a reply missing
 * them degrades to looking like a v1 report, not a failed parse.
 */
export function parseDailyReportV2(rawResponse: string): ParsedAIReport {
  ...
  const content: AIReportContent = {
    ...
    rawResponse,
    estimatedCalories: optionalNumber(obj?.estimatedCalories),
    proteinG: optionalNumber(obj?.proteinG),
    carbsG: optionalNumber(obj?.carbsG),
    fatG: optionalNumber(obj?.fatG),
    fiberG: optionalNumber(obj?.fiberG),
    calorieBalanceText: optionalString(obj?.calorieBalanceText),
    calorieBalanceKcal: optionalNumber(obj?.calorieBalanceKcal),
    musclesTrained: optionalStringArray(obj?.musclesTrained),
    workoutDurationMin: optionalNumber(obj?.workoutDurationMin),
    workoutCaloriesBurned: optionalNumber(obj?.workoutCaloriesBurned),
    workoutExercises: optionalExercises(obj?.workoutExercises),
  };
